<?php
     ...
     if ($bydirect) {
          if (isset($_POST['name'])) {
               ...
               if ($validation_flag == 0) {
                    $query = "INSERT INTO `classes` (`spe_id`,`category_id`,`name`,`description`,`timezone`,`latitude`,`longitude`,`address`,`city`,`state`,`country`,`zipcode`,`is_paid`,`price`,`image`,`banner_image`,`discount_type`,`discount_amount`,`offer_start_date`,`offer_end_date`,`status`,`created`,`created_by`) VALUES ( '".$spe_id."', '" . $category_id . "','" . $name . "','" . $description . "','" . $timezone . "','" . $latitude . "','" . $longitude . "','" . $address . "','" . $city . "','" . $state . "','" . $country . "','" . $zipcode . "','" . $is_paid . "','" . $price . "','" . $logo . "','" . $banner . "','" . $discount_type . "','" . $discount_amount . "'," . $offer_start_date . "," . $offer_end_date . ",'" . $status . "','" . $created . "','" . $created_by . "');";
                    $query_result = $con->query($query);
                    $class_id = $con->insert_id;
                    if(!empty($instructors)){
                         foreach($instructors as $instructor){
                              $query_in = "INSERT INTO `class_instructors` (`class_id`,`user_id`,`status`,`created`,`created_by`) VALUES ( '".$class_id."', '" . $instructor . "','1','" . date('Y-m-d H:i:s') . "','" . $created_by . "');";
