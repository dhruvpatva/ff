...
if ($bydirect) {
     if (isset($_REQUEST['name'])) {
          ...
          $query = "SELECT id FROM categories WHERE type='Events' AND name='$name'";
          $query_result = $con->query($query);
          if ($query_result->num_rows == 0) {
               $query = "INSERT INTO categories (type, name, description, created, status) VALUES ('Events', '".$name."', '".$description."', '".date("Y-m-d H:i:s")."', '".$status."')";
               $query_result = $con->query($query);
               $result['success'] = 1;
               $result['data'] = 'success';
               $result['error'] = 0;
