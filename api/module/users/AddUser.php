...
if (isset($_SESSION['user']) && $_SESSION['user']['userRole'] == "admin"){ 
     ...
     if(isset($_REQUEST['addspi']) && $_REQUEST['addspi'] == 1){  
         if (isset($_REQUEST['firstname']) && isset($_REQUEST['lastname']) && isset($_REQUEST['email'])){
               $validation_flag = 0;
               $validation_error_code = NULL;
               $first_name = $_REQUEST['firstname'];
               ...
               if ($validation_flag == 0) {
                   ...
                   $query_result = $con->query($query);
                   if ($query_result->num_rows == 0) {
                               $query = "INSERT INTO `users` (role_id, email, password, firstname, lastname, profile_image, gender, dob, mobile, aboutus, profile_cover, timezone, city, state, country, zipcode, address, latitude, longitude, status) 
                                        VALUES ('$role_id','$email', '$password', '$first_name', '$last_name', '$profilepic', '$gender', '$dob','$phone', '$aboutus', '$profile_cover','$timezone','$city','$state', '$country', '$zipcode', '$address', '$latitude', '$longitude','$status')";
